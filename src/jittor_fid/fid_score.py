...
"""
import jittor as jt
from jittor import nn, transform as TF
import os
import pathlib
...
from jittor_fid.inception import InceptionV3

parser = ArgumentParser(formatter_class=ArgumentDefaultsHelpFormatter)
parser.add_argument('--batch-size', type=int,
                    default=50, help='Batch size to use')
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-06):
    ...
    assert (mu1.shape
            == mu2.shape), 'Training and test mean vectors have different lengths'
    assert (sigma1.shape
            == sigma2.shape), 'Training and test covariances have different dimensions'
    diff = (mu1 - mu2)
...
def compute_statistics_of_path(path, model, batch_size, dims, num_workers=1):
    if path.endswith('.npz'):
        ...
        (m, s) = calculate_activation_statistics(
            files, model, batch_size, dims, num_workers)
    return (m, s)
...
def calculate_fid_given_paths(paths, batch_size, dims, num_workers=1):
    ...
    (m1, s1) = compute_statistics_of_path(
        paths[0], model, batch_size, dims, num_workers)
    (m2, s2) = compute_statistics_of_path(
        paths[1], model, batch_size, dims, num_workers)
...
def main():
    ...
    fid_value = calculate_fid_given_paths(
        args.path, args.batch_size, args.dims, num_workers)
    print('FID: ', fid_value)
